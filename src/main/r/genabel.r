...
#analyzeEmpSnps <- function(model, data, data.summary, times=50, top=20, trait.type='guess', sort='Pc1df', show.plot=T)
#{
#	data.qt <- emp.qtscore(model, data, times=times, trait.type=trait.type, quiet=F)
#	topsnps <- descriptives.scan(data.qt, top=top, sort=sort)
#	snptable <- extractSnpTable(topsnps,data.summary,cutoff=1)#,p.value.col=sort)
#	#print(snptable)
#	if (show.plot)
#		plot(data.qt, df="Pc1df")
#	return(snptable)
...
addSnpsToPhdata <- function(data,snps)
{
	...
	{
		#sapply(as.character(data5[,'rs6816710']),function(value){sub('/', '', value)})
		#sapply(as.character(data[,snp]),function(value){sub('/', '', value)})
		#phdata[[snp]] <- as.factor(as.character(data[,snp]))
